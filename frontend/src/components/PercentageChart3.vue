...
<div class="col-md-12 col-md-offset-2">
  <!-- 사용자와 동일 성별/연령 비교(percentageChart3)의 결과 글로 나타내기 -->
...
<script>
// 동일 성별/연령 비교 차트(기간 : 현재 날짜로 부터 한달전) --> percentageChart3
        ...
        return {
          ...
          created() {
                ...
                if (!this.$session.exists()) {
                  console.log('********** 세션없음 (percentageChart3.vue) **********');
                } else {
                  console.log('********** 세션있음 (percentageChart3.vue) **********');
                  ...
                  console.log('percentage3 start_date 날짜는 ' + start_date);
                  console.log('percentage3 end_date 날짜는 ' + end_date);
                  setTimeout(function() {
                    //DB 조회 후 현재날짜로 부터 지난 한달간 소비내역 데이터 response받기. (동일 성별/연령)
                    axios({
                      method: 'post',
                      ...
                      for (var k = 0; k < compare_other.length; k++) {
                        ...
                        //카테고리별 소비내역 비율 지정 후 차트에 적용.(동일 성별/연령)
