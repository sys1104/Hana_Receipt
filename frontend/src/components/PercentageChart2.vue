...
<!-- 전체 이용자 평균 소비내역(percentageChart2)의 결과 글로 나타내기 -->
        ...
        return {
          ...
          created() {
                ...
                if (!this.$session.exists()) {
                  console.log('********** 세션없음 (percentageChart2.vue) **********');
                } else {
                  console.log('********** 세션있음 (percentageChart2.vue) **********');
                  ...
                  console.log('---- percentageChart2의 start_date는 : ' + start_date);
                  console.log('---- percentageChart2의 end_date는 : ' + end_date);
                  setTimeout(function() {
                    //DB 조회 후 현재날짜로 부터 지난 한달간 소비내역 데이터 response받기. (전체 이용자)
                    axios({
                      method: 'post',
                      ...
                      for (var k = 0; k < compare_other.length; k++) {
                        ...
                        //카테고리별 소비내역 비율 지정 후 차트에 적용.(전체 이용자)
